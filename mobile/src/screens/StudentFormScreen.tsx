...
export default function StudentFormScreen({ studentId }: Props) {
  ...
  const validate = (): boolean => {
    ...
    if (!isEditing && !password.trim()) {
      setError("A senha é obrigatória para novos alunos.");
      return false;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isEditing) {
        ...
        await UserService.update(studentId, payload);
      } else {
  ...
  return (
    ...
    >
      <Stack.Screen
        options={{ title: isEditing ? "Editar Aluno" : "Novo Aluno" }}
      />
